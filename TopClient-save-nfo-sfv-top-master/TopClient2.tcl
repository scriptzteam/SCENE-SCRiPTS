...
proc Get_NFO_On_FTP {chan nick top_name Release_Name Release_Section file_name} {
        ...
        if {[check_url $chan $nick $Release_Name $File_md5]} {
        }
}
proc check_url { chan nick Release_Name File_md5 } {
	if { [catch { set	URL	[exec -- egrep -o -e {(http[s]?://)?(www\.)([a-zA-Z0-9][a-zA-Z0-9\.-]{2,70}[a-zA-Z0-9\]\.[a-zA-Z]{2,3})(/)?[a-zA-Z0-9/\._~:?#\@=\!$&'()*+,;]+?} "/var/www/tt/$File_md5"] } error] } {
		if { [string match "*child*process*exited*abnormally*" $error] } {
			putlog "stop check url is not found in $File_md5";
		} else {
			putlog "Error url : $error";
		}
	} else {
		foreach URL_LINK $URL {
			if { ![string match -nocase "http*://*" $URL_LINK] } { set URL_LINK "http://$URL_LINK"; }
			putlog "addurl --->> $Release_Name $URL_LINK";
                        proc_SendMSG $chan $nick addurl "$Release_Name $URL_LINK";
		}
	}
}
