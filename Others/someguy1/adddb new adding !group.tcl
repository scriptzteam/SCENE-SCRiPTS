...
set script_version "ADD ALL || S3ARCH L33T VERSiON SCRiPT v5.04"
...
bind pub - !last isy:last
bind pub - !group isy:grp
	...
	if {[channel get $chan $type]} {
		...
		if { $rlsvalid == "1" } {
			...
			if {$isdone == "1"} {
				...
				putnow "NOTICE $chan :!$type $rlsname $reason $network"
...
proc isy:spamswitch {chan rlsname unixtime reason network status type} {
	...
	if {[channel get $chan $type]} {
		...
		if {$isdone == "1"} {
			...
			putnow "PRIVMSG #aa :!$type $rlsname $reason $network"
...
proc isy:searchdb {chan query nick limit type} {
	...
	if {[channel get $chan $type]} {
		...
		if {$results != ""} {
			...
			if {$nick != ""} {
				putnow "PRIVMSG $chan :$bopen \00314Sending last $limit results to\003 $bclose\00312-\003$bopen \0037$nick\003 $bclose"
				set chan "$nick"
			}
            ...
            foreach { rlsname sec time files size genre reason network status nfoid sfvid m3uid jpgid coverid mp3info videoinfo url } $results {
				...
				putnow "PRIVMSG $chan :$bopen$count$bclose $bopen$section$bclose $rlsname $infod$genred$predago$menuked$nfod$sfvd$m3ud$jpgd$coverd$mp3infod$videoinfod$urld"
			
			}

		} else {
			
			putnow "PRIVMSG $chan :Nothing found for $query"	
...
proc isy:last {nick uhost handle chan arg} {
	return [isy:searchdb $chan $arg "" "5" "search"];
}

proc isy:grp {nick uhost handle chan arg} {
	return [isy:grpinfo $chan $arg "search"];
}

proc isy:grpinfo {chan query type} {
	global db_handle bopen bclose bdiv
...
# It would be nice if you didn't delete this but there is really nothing I can do!
putlog "ADD NF0|SFV|M3U|JPG|DIZ v2.01 --> By \002sCRiPTzTEAM\002 ||| Loaded Succesfully!"