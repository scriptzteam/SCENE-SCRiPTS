...
# It took many many long hours to code this script, please leave credit where its due
package require mysqltcl
package require TclCurl
 
set script_version "ALL v2.12 SCRiPT"
...
set chann_(sitepre) "#sitepre"

set chann_(addpre) "#addpre"
set chann_(addinfo) "#addpre"
set chann_(nuke) "#nukes"
set chann_(nuke2) "#nukes"
set chann_(delpre) "#nukes"

set chann_(addmp3) "#addid3c"
set chann_(addvideo) "#videoinfo"
set chann_(addurl) "#addurl"

set chann_(addold) "#addold"
set chann_(sitepre) "#sitepre"

# Set this to your IRC pre announce channels.
set chann_(infoann) "#addpre"
set chann_(spamann) "#addpre"

set chann_(addspam) "#addnfo"

set addfeedurl "http://XX/preadddata.php?data="
...
proc spreadpreaffil {bot com args} {
	...
	putquick "PRIVMSG $chann_(sitepre) :!sitepre $rlsname $section $files $size"
...
proc nfoprerls {bot com args} {
	...
	if { $bot == $whichbot } {	
		...
		putquick "PRIVMSG $chann_(addspam) :!addnfo $rlsname https://XXURL/get.php?r=$hash $filename $crc"
...
proc sfvprerls {bot com args} {
	...
	if { $bot == $whichbot } {	
		...
		putquick "PRIVMSG $chann_(addspam) :!addsfv $rlsname https://XXURL/get.php?r=$hash $filename $crc"
...
proc m3uprerls {bot com args} {
	...
	if { $bot == $whichbot } {	
		...
		putquick "PRIVMSG $chann_(addspam) :!addm3u $rlsname https://XXURL/get.php?r=$hash $filename $crc"
...
proc jpgprerls {bot com args} {
	...
	if { $bot == $whichbot } {	
		...
		putquick "PRIVMSG $chann_(addspam) :!addjpg $rlsname https://XXURL/get.php?r=$hash $filename $crc"
...
proc dizprerls {bot com args} {
	...
	if { $bot == $whichbot } {	
		...
		putquick "PRIVMSG $chann_(addspam) :!adddiz $rlsname https://XXURL/get.php?r=$hash $filename $crc"
	
	}
}

proc isy:sitepre { nick uhost hand chan arg } {
	global mysql_ predb_ chann_ db_handle PR3N3T whichbot 
	
	if { $chan == $chann_(sitepre) } { 
		...
		set files [string trim [lindex $arg 2]]
		set size [string trim [lindex $arg 3]]
		...
		set grp [string trim [lindex [split [lindex $arg 0] "-"] end]]
			...
			if { $result == "0" } {
				...
				set fromdata "$nick:$chan:$PR3N3T:SITEPRE"
				set frominfo [::mysql::escape $fromdata]
				
				set isdone [mysqlexec $db_handle "INSERT INTO $whichtable ( `$predb_(sectionmain)` , `$predb_(section)` , `$predb_(rlsname)` , `$predb_(unixtime)` , `$predb_(grp)` , `$predb_(addedon)` , `$predb_(frompre)` , `$predb_(files)` , `$predb_(size)` ) VALUES ( '$sectionmain' , '$section' ,  '$rlsname' , '$unixtime' ,  '$grp' , '$unixtime' , '$frominfo' , '$files' , '$size' )"]
					
				if { $isdone == "1" && $rlsvalid == "1" } {
					
					#putbot $whichbot "SITEPRE $rlsname $section $grp"
...
proc nfoinsidedb { nick uhost hand chan args } {
	...
	if { $hashdata != "" } {
		...
		#putbot "ABCD" "ADDNFO $rlsname $hash $filename $crc $size"
...
proc sfvinsidedb { nick uhost hand chan args } {
	...
	if { $hashdata != "" } {
		...
		#putbot "ABCD" "ADDSFV $rlsname $hash $filename $crc $size"
...
proc m3uinsidedb { nick uhost hand chan args } {
	...
	if { $hashdata != "" } {
		...
		#putbot "ABCD" "ADDM3U $rlsname $hash $filename $crc $size"
