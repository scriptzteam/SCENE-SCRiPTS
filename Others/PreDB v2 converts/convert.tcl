...
proc isy:dopre { count } {
	global mysql_ db_handle
		...
		set query1 [mysqlsel $db_handle "SELECT rel_name,rel_time,rel_files,rel_size,rel_genre FROM allpres" -flatlist];
        
		if {$query1 != ""} {
			
            foreach {rlsname rlstime files size genre} $query1 {
				
				set rlsname [string trimleft [string trimright [string trimleft [string trimright [string trim [isy:trimcolors $rlsname]] "."] "."] "_"] "_"]
				set genre [string trim [isy:trimcolors $genre]]
				
				set grp [string trim [lindex [split $rlsname "-"] end]]
				
				# regsub -all {\.} $grp "_" grp
				...
				if { $rlsvalid == "1" && $grpvalid == "1" } { 
					...
					puts "$rlsvalid $grpvalid $rlsname $grp"
					
				}
				
				set isdone [mysqlexec $db_handle "INSERT INTO $whichtable ( rlsname,section,unixtime,files,size,genre,grp) VALUES ( '[::mysql::escape $rlsname]', '[::mysql::escape $section]', '[::mysql::escape $dateon]', '[::mysql::escape $files]', '[::mysql::escape $size]', '[::mysql::escape $genre]', '[::mysql::escape $grp]' ) ON DUPLICATE KEY UPDATE unixtime = IF(VALUES(unixtime) < unixtime, unixtime, VALUES(unixtime)), files = IF(VALUES(files) < files, files, VALUES(files)), size = IF(VALUES(size) < size, size, VALUES(size))"]
