...
$APPENDIMDB = 0;												##
##														##
## In that case, what file? preferably a .something file so the ftp-client reads it				##
$THEMESSAGEFILE = ".imdbdata";											##
##														##
...
##														##
## Ignore dirs, relative to site dir (case sensitive)								##
## Just put the folder name here. NO PATH, just the foldername. If you want to ignore /site/VIDEO/GROUPS	##
## put GROUPS here.												##
$IGNOREDIR1 = "Groups";												##
$IGNOREDIR2 = "Admin";												##
$IGNOREDIR3 = "pre";												##
...
sub readurl {
	...
	foreach  $thing (@NFO) {
		...
		$pathtorel =~ s/\"//g;
		$pathtorel =~ s/IMDB: //;
		($crp, $site, $dir, $otherdir) = split("/", $pathtorel);
		if ($dir ne $IGNOREDIR1 and $dir ne $IGNOREDIR2 and $dir ne $IGNOREDIR3 and $otherdir ne $IGNOREDIR1 and $otherdir ne $IGNOREDIR2 and $otherdir ne $IGNOREDIR3) {
...
sub getnfo {	
	...
	if ($1 eq $genre) {$plot = "Unavailable..."} else {$plot = $1; $plot =~ s/\"//g;};
	# User Rating
	$data2 =~ /<\/A> <B>(\d*)\.(\d*)<\/B>\/10 \((\d*)/;
	if ($1 ne $plot and $1 ne $genre) {$rating = "$1.$2 / 10 with $3$4 votes"; $rrating = $1} else {$rating = "N/A"};
	# Opening weekend
...
sub domessg {
	if ($APPENDIMDB == 1) {
	$pathtorel =~ s/ $//;
	$messagefile = $pathtorel."/$THEMESSAGEFILE";
...
sub findnprint {
	...
	if ($test eq "IMDb title search") {
		...
		if ($imdbnum ne "") {
			...
			$url2 = "http://us.imdb.com/Title?$imdbnum";			
	...
	if ($test ne "Invalid title or title code" and $test ne "IMDb title search") {
		### Pulling out shit
		# Finding imdbnum
		$data2 =~ /<BASE HREF=\"http:\/\/us\.imdb\.com\/Title\?(\d*)\">/;
		$imdbnum = $1;
		$url2 = "http://us.imdb.com/Title?$imdbnum";
		# Title
		$data2 =~ /<TITLE>(.*)<\/TITLE>/;
