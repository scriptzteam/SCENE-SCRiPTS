...
class Blowfish:
    ...
    def __init__(self, key=None):
        self.key = key
        if key and len(key) > 72:
            self.key = self.key[:72]

    def encrypt(self, plaintext):
        if not self.key: return
        size = len(plaintext) * 2 + 1
        cciphertext = create_string_buffer(size)
        bfLib.encrypt_string(c_char_p(self.key), c_char_p(plaintext), 
                             cciphertext, len(plaintext))
        cciphertext[size - 1] = '\0'
        return string_at(cciphertext, size)

    def decrypt(self, ciphertext):
        if not self.key: return
        size = len(ciphertext) * 2 + 1
        cplaintext = create_string_buffer(size)
        bfLib.decrypt_string(c_char_p(self.key), c_char_p(ciphertext),
                           cplaintext, len(ciphertext))
        cplaintext[size - 1] = '\0'
        return string_at(cplaintext, size)


def blowcrypt_pack(msg, cipher):
    """."""
    return '+OK ' + cipher.encrypt(msg)
...
def fish_modifier_in_notice_cb(data, modifier, server_name, string):
    ...
    target = "%s/%s" % (server_name, match.group(2).lower())
...
def fish_modifier_in_privmsg_cb(data, modifier, server_name, string):
    ...
    if match.group(3) == weechat.info_get("irc_nick", server_name):
        ...
        dest = match.group(3)
    target = "%s/%s" % (server_name, dest.lower())
...
def fish_cmd_blowkey(data, buffer, args):
    ...
    target = "%s/%s" % (server_name, target_user.lower())
