...
# --------------------------------------------------------------------------- #
# sorted-links-by-year.sh (v0.1)                                   2002-09-18 #
#                                                                             #
...
#              symlinks to the mp3 releases found based on the album year.    #
#                                                                             #
...
DEST="/site/incoming/_sorted/mp3.by.year"
...
# --------------------------------------------------------------------------- #
# get the album copyright year.                                               #
# --------------------------------------------------------------------------- #

			FILE="$( ls -1 "${FTPROOT}${SOURCE}/${DAY}/${DIR}" | grep "\.mp3" | head -1 )"

			YEAR="$( ${MPINFO} -p "%y" "${FTPROOT}${SOURCE}/${DAY}/${DIR}/${FILE}" )"

# --------------------------------------------------------------------------- #
# Create year folder if it doesn't exist.                                     #
# --------------------------------------------------------------------------- #

			if [ -e "${FTPROOT}${DEST}/${YEAR}" ] ;
			then
				YEAR_DIR=1
			else
				mkdir -m755 "${FTPROOT}${DEST}/${YEAR}"
			fi
...
# --------------------------------------------------------------------------- #
			...
			ln -s "${SOURCE}/${DAY}/${DIR}" "${FTPROOT}${DEST}/${YEAR}/${DIR}" >/dev/null 2>&1
		fi

	done
done

YEARS="$( ls "${FTPROOT}${DEST}" | egrep -v ${SKIP} )"
...
# --------------------------------------------------------------------------- #
	...
	REL_COUNT=`ls -1 "${FTPROOT}${DEST}/${YEAR}" | wc -l | xargs`
	...
	for TARGET in `ls -l "${FTPROOT}${DEST}/${YEAR}" | grep -v "total " | cut -b 57- | awk -F' ' '{ print $3 }'` ;
	do
...
# --------------------------------------------------------------------------- #
	...
	then
		...
		OLD_HEADER=`ls -a1 "${FTPROOT}${DEST}/${YEAR}/" | egrep "RELEASE|MBytes"`

		NEW_HEADER="${HEADER_PART_A}${HEADER_PART_B}"

		if [ "${OLD_HEADER}" == "" ] ;
		then
			mkdir -m755 "${FTPROOT}${DEST}/${YEAR}/${NEW_HEADER}"
		else
			rmdir "${FTPROOT}${DEST}/${YEAR}/${OLD_HEADER}"
			mkdir -m755 "${FTPROOT}${DEST}/${YEAR}/${NEW_HEADER}"
		fi
	else
		rm -rf "${GROUP}${DEST}/${YEAR}/"
	fi
done
