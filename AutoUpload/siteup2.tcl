...
set isy_(login_username) "xxx"
set isy_(login_password) "xxx"
set isy_(login_key) "xxx"

set isy_(login_url) "http://example.com/login.php?k=xxx"
...
proc isy:getcategoryid { release section } {
	...
	if { $section == "X264" } {
		...
		if {[regexp -nocase {[\.\-\_](1080i|1080p)[\.\-\_]} $release]} { set id "5" }
...
global isy_ createtrtbotnick 
		...
		curl::transfer -url $isy_(takelogin_url) -timeout $isy_(timeout) -verbose 1 -post 1 -httppost [list name "username" contents $isy_(login_username)] -httppost [list name "password" contents $isy_(login_password)] -httppost [list name "loginkey" contents $isy_(login_key)] -followlocation 1 -maxredirs 1 -useragent $isy_(agent) -cookiefile $isy_(cookie_path) -cookiejar $isy_(cookie_path) -referer $isy_(login_url) -httpheader "Expect: "
		
		curl::transfer -url $isy_(takeup_url) -timeout $isy_(timeout) -bodyvar downid -verbose 1 -post 1 -httppost [list name "submit"  contents "true"] -httppost [list name "uplver"  contents "yes"] -httppost [list name "type" contents $postdata_type] -httppost [list name "nfo" file "$postdata_nfo"] -httppost [list name "file" file "$postdata_file"] -httppost [list name "showid" contents "1"] -httppost [list name "MAX_FILE_SIZE" contents "5000000"] -followlocation 1 -maxredirs 2 -useragent $isy_(agent) -cookiefile $isy_(cookie_path) -cookiejar $isy_(cookie_path) -referer $isy_(up_url) -httpheader "Expect: "
...
putlog "$isy_(site_name) eAutoUp-D0WN FiSH v1.86 By Islander -> Loaded Successfully."